...
- **Portfolio vs S&P 500** — you type each year's return for the portfolio and the S&P in Admin → Annual returns. The overview shows a per-year bar chart plus the **annualized (compound/CAGR) return** for each, computed from those yearly figures over the years entered (with cumulative return too). Nothing is fetched.
